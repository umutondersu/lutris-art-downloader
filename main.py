...
# dependencies = ["inquirer>=2.10.0", "requests>=2.27.1"]
# ///
...
def SetAPIKey():
    ...
    print(
        "You can get one by using your Steam account and heading here: https://www.steamgriddb.com/profile/preferences/api\n"
    )
...
def TestAPI(key, api):
    r = requests.get(
        "https://www.steamgriddb.com/api/v2/grids/game/1?dimensions=600x900",
        headers=key,
    )
...
def DBConnect():
    try:
        ...
        print(
            "Could not find Lutris database 'pga.db'. You can manually edit script's path if necessary"
        )
        exit(1)
    return conn
...
def SearchGame(game):
    res = requests.get(
        "https://www.steamgriddb.com/api/v2/search/autocomplete/" + game, headers=auth
    ).json()
    if len(res["data"]) == 0:
